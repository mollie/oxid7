...
class Order extends Order_parent
{
    ...
    public function mollieMarkOrderAsShipped()
    {
        ...
        try {
            ...
            if ($oMollieApiOrder instanceof \Mollie\Api\Resources\Order) {
                ...
                $oRequestLog->logRequest([], $oResponse, $this->getId(), Registry::getConfig()->getShopId());
    ...
    {
        if ($this->blMollieFinalizeReturnMode === false) {
            $mParentReturn = parent::setPayment($sPaymentid);

            if ($this->mollieIsMolliePaymentUsed() && $this->mollieRecalculateOrder === false) { // dont change these in recalculation mode
    ...
    public function mollieSendOrderByEmail()
    {
        ...
        parent::sendOrderByEmail($oUser, $oBasket, $oPayment);
        $this->mollieSetOrderEmailSendDate();
    }
    ...
    protected function assignUserInformation($oUser)
    {
        if ($this->blMollieIsApplePayButtonMode === false) {
            return parent::assignUserInformation($oUser);
        }
        ...
        if ((bool)$oPaymentModel->getConfigParam('ignore_apple_pay_delivery_address') === true && !empty($oDelAdress)) {
            ...
            $this->oxorder__oxdelstateid = new Field(
                UserHelper::getInstance()->getStateFromAdministrativeArea($aShippingContact['administrativeArea'])
            );
            $this->oxorder__oxdelzip = new Field($aShippingContact['postalCode']);
            $this->oxorder__oxdelsal = new Field(
                UserHelper::getInstance()->getSalByFirstname($aShippingContact['givenName'])
            );
    ...
    {
        if (!$oMollieApiOrder instanceof \Mollie\Api\Resources\Payment || (bool)Payment::getInstance()->getShopConfVar('sMollieAutomaticRefundOnCancel') === false) {
            return false;
        }
    ...
    protected function mollieRefundPayment($oMollieApiOrder, $sCurrency)
    {
        ...
        Api::getInstance()->logApiRequest($aParams, $oResponse, $this->getId(), Registry::getConfig()->getShopId());
    }
    ...
    protected function mollieCancelPayment($oApiEndpoint)
    {
        ...
        Api::getInstance()->logApiRequest([], $oResponse, $this->getId(), Registry::getConfig()->getShopId());
    }
