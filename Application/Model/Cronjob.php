...
class Cronjob
{
    ...
    public function isCronjobAlreadyExisting($sCronjobId, $iShopId)
    {
        ...
        if (!DatabaseProvider::getDb()->getOne($sQuery, [$sCronjobId, $iShopId])) {
            return false;
        }
        return true;
    }
    ...
    {
        DatabaseProvider::getDb()->execute("UPDATE `".self::$sTableName."` SET LAST_RUN = NOW() WHERE OXID = ? AND OXSHOPID = ?;", [$sCronjobId, $iShopId]);
    }
    ...
    public function getCronjobData($sCronjobId, $iShopId)
    {
        ...
        return $oDb->getRow($sQuery, [$sCronjobId, $iShopId]);
    }
}