<?php

$sLangName  = "Deutsch";

// -------------------------------
// RESOURCE IDENTIFIER = STRING
// -------------------------------
$aLang = [
    'charset'                                   => 'UTF-8',
    'MOLLIE_LOCALE'                             => 'de_DE',
    'MOLLIE_SHIPPINGCOST'                       => "Versandkosten",
    'MOLLIE_PAYMENTTYPESURCHARGE'               => "Zahlungsart-Aufschlag",
    'MOLLIE_WRAPPING'                           => "Geschenkverpackung",
    'MOLLIE_GIFTCARD'                           => "Grußkarte",
    'MOLLIE_VOUCHER'                            => 'Gutschein',
    'MOLLIE_DISCOUNT'                           => 'Rabatt',
    'MOLLIE_ROUNDINGCORRECTION'                 => 'Rundungskorrektur',
    'MOLLIE_SELECT_BANK'                        => 'Bank auswählen',
    'MOLLIE_SELECT_GIFTCARD'                    => 'Geschenkkarte auswählen',
    'MOLLIE_PLEASE_SELECT'                      => '-- Bitte wählen --',
    'MOLLIE_QR_CODE'                            => 'QR Code',
    'MOLLIE_SINGLE_CLICK_ACCEPTED'              => 'Speicherung der Kreditkarten-Daten erlauben (optional)',
    'MOLLIE_SINGLE_CLICK_ACCEPTED_HAS_CUSTOMER_ID' => 'Verwendung gespeicherter Kreditkarten-Daten erlauben (optional)',
    'MOLLIE_SINGLE_CLICK_INFO'                  => 'Hiermit bestätigen Sie die sichere Speicherung Ihrer Kreditkarten-Daten aufseiten des Zahlungsanbieters bzw. die Verwendung Ihrer vorher dort gespeicherter Daten.',

    'MOLLIE_ERROR_ORDER_NOT_FOUND'              => 'Bestellung konnte nicht gefunden werden',
    'MOLLIE_ERROR_TRANSACTIONID_NOT_FOUND'      => 'Transaktions-Id konnte nicht gefunden werden',
    'MOLLIE_ERROR_SOMETHING_WENT_WRONG'         => 'Ein unbekannter Fehler ist aufgetreten',
    'MOLLIE_ERROR_ORDER_CANCELED'               => 'Die Bezahlung wurde storniert, bitte versuchen Sie es erneut',
    'MOLLIE_ERROR_ORDER_FAILED'                 => 'Die Bezahlung ist fehlgeschlagen, bitte versuchen Sie es erneut',
    'MOLLIE_BILLING_APPLE_PAY_NOT_AVAILABLE'    => 'Apple Pay ist für diese Rechnungsadresse nicht verfügbar',
    'MOLLIE_SECOND_CHANCE_MAIL_SUBJECT'         => 'Abschluss Ihrer Bestellung bei',

    'MOLLIE_NOT_AVAILABLE_BASKETSUMLIMIT'       => 'Die Zahlart ist mit dem aktuellen Bestellwert nicht nutzbar.',
    'MOLLIE_NOT_AVAILABLE_COUNTRY'              => 'Die Zahlart ist für das gewählte Rechnungsland nicht verfügbar.',
    'MOLLIE_NOT_AVAILABLE_B2B'                  => 'Die Zahlart steht nur für B2B Bestellungen zur Verfügung.',
    'MOLLIE_NOT_AVAILABLE_CURRENCY'             => 'Die Währung wird von der Zahlart nicht unterstützt.',
    'MOLLIE_CHANGE_CURRENCY'                    => 'Währung wechseln',
    'MOLLIE_SUPPORTED_CURRENCIES'               => 'Unterstützte Währungen',

    'MOLLIE_PAYPAL_EXPRESS_DESCRIPTION'             => 'Zahlung bei ',
    'MOLLIE_PAYPAL_EXPRESS'                         => 'PayPal Express',
    'MOLLIE_PAYPAL_EXPRESS_INFO'                    => 'Ihre Zahlung wird mit PayPal Express durchgeführt.<br>Um Ihre PayPal Express Zahlung abzubrechen und eine andere Zahlart auszuwählen, klicken Sie auf den "Aufheben" Button.',
    'MOLLIE_PAYPAL_EXPRESS_UNLINK'                  => 'Aufheben',
    'MOLLIE_PAYPAL_EXPRESS_SESSIONID_MISSING'       => 'Ein Fehler ist aufgetreten. Informationen zu PayPal Express Transaktion konnten nicht gefunden werden, bitte versuchen Sie es erneut.',
    'MOLLIE_PAYPAL_EXPRESS_NO_SESSION_INFO'         => 'Ein Fehler ist aufgetreten. Informationen zu PayPal Express Transaktion konnten nicht abgerufen werden, bitte versuchen Sie es erneut.',
    'MOLLIE_PAYPAL_EXPRESS_ADDRESS_DATA_INCOMPLETE' => 'Ein Fehler ist aufgetreten. Die von PayPal gelieferte Adresse ist nicht vollständig. Bitte korrigieren Sie Ihre Adresse in PayPal oder wählen Sie eine andere Zahlart.',
    'MOLLIE_PAYPAL_DELIVERY_COUNTRY_INACTIVE'       => 'Ihr ausgewähltes Lieferland kann von uns aktuell leider nicht beliefert werden.',

    'MOLLIE_CREDIT_CARD_HOLDER'                             => 'Karteninhaber',
    'MOLLIE_CREDIT_CARD_NUMBER'                             => 'Kartennummer',
    'MOLLIE_CREDIT_CARD_VALID_UNTIL'                        => 'Gültig bis',
    'MOLLIE_CREDIT_CARD_SECURITY_CODE'                      => 'Prüfziffer',
    'MOLLIE_CREDIT_CARD_SECURITY_CODE_DESCRIPTION'          => 'Diese befindet sich auf der Rückseite Ihrer Kreditkarte. Die Prüfziffer sind die letzten drei Ziffern im Unterschriftsfeld.'
];
